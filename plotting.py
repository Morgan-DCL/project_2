    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color="#006b3c", line=dict(color="black", width=1)
                ),
...
def directors_top_1_by_decades(df: pd.DataFrame):
    grouped_df = (
        df.groupby(["cuts", "person_name"], observed=True)
        .size()
        .reset_index(name="total_film_directors")
        .sort_values(by="total_film_directors")
    )

    top_directors_decennie = (
        grouped_df.groupby("cuts", observed=True)
        .apply(lambda x: x.nlargest(1, "total_film_directors"))
        .reset_index(drop=True)
    )
